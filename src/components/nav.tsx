...
import { Home, Mail, List, BookOpen, Sparkles, Heart, Crown, Timer } from "lucide-react";
...
export default function SectionNav() {
    ...
    return (
        <>
                ...
                <ul className="flex flex-1 flex-col gap-0.5 px-3 py-2">
                    {sidebarSections.map((s, i) => {
                        ...
                        const showGroupDivider =
                            i > 0 && sidebarSections[i - 1].group !== s.group;
                                    ...
                                    <span className="flex items-center gap-2">
                                        <span
                                            className={`inline-block h-1 w-1 flex-shrink-0 rounded-full transition-colors duration-200 ${
                                                isActive ? "bg-[#C4A882]" : "bg-transparent"
                                            }`}
                                        />
                                        {s.label}
                                    </span>
            ...
            >
                <div
                    className="flex items-stretch justify-start overflow-x-auto border-t border-[#D4C4AE]/60 bg-[#F5F0E8]/96 shadow-lg backdrop-blur-md"
                    ...
                    {bottomBarSections.map(({ id, label, Icon }) => {
                        ...
                        return (
                            ...
                            >
                                <Icon
                                    size={17}
                                    className={isActive ? "text-[#6B5544]" : "text-[#C4A882]/70"}
                                    strokeWidth={isActive ? 2 : 1.5}
                                />
                                <span
                                    className={`text-[8.5px] tracking-wide ${
                                        isActive ? "font-medium text-[#6B5544]" : "text-[#B8A898]"
                                    }`}
                                >
                                    {label}
                                </span>
