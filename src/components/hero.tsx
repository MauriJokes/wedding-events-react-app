...
export default function Hero({ onEnter, onOpened }: { onEnter?: () => void; onOpened?: () => void }) {
    ...
    return (
        ...
        >
            ...
            <motion.div
                ...
                style={{ backgroundColor: "rgba(212, 195, 174, 0.88)", willChange: "transform" }}
                animate={isOpen ? { x: "-110%" } : { x: 0 }}
                transition={{ duration: 1.3, ease: [0.4, 0, 0.15, 1] }}
                onAnimationComplete={() => { if (isOpen) { setHidden(true); onOpened?.(); } }}
            >
                {/* Vertical center stripe on right edge */}
                <div
                    className="absolute inset-y-0 right-0"
                    style={{ width: "56%", maxWidth: 160, minWidth: 100, backgroundColor: "#EDE6DC" }}
                />
            </motion.div>
            ...
            <motion.div
                ...
                animate={isOpen ? { x: "-60vw", opacity: 0 } : { x: 0, opacity: 1 }}
                transition={{ duration: 1.3, ease: [0.4, 0, 0.15, 1] }}
            >
                <div
                    className="relative flex items-center justify-center rounded-full shadow-md"
                    style={{ width: 168, height: 168, backgroundColor: "#C4A882" }}
                >
            ...
            <motion.button
                onClick={handleOpen}
                className="absolute bottom-48 left-1/2 -translate-x-1/2 whitespace-nowrap border border-[#6B5544]/60 px-5 py-2 text-[10px] tracking-[0.3em] text-[#6B5544]/60 uppercase transition-colors hover:border-[#6B5544] hover:text-[#6B5544] active:scale-[0.97]"
